...
commit;
select * from Loan;
select c.customername from BankCustomer c where exists(select d.customername, count(d.customername) from Depositor d,BankAccount a where d.accno=a.accno and c.customername=d.customername and a.branchname='SBI_ResidencyRoad' group by d.customername having count(d.customername)>=2);
select d.customername from BankAccount a,Branch b,Depositor d where b.branchname=a.branchname and a.accno=d.accno and b.branchcity='Delhi' group by d.customername having count(distinct b.branchname)=(select count(branchname) from Branch where branchcity='Delhi');
